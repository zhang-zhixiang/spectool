...
class MatchResult(object):
    def __init__(self):
        self.fullpath_tempname = None
        self.temperature = 4444
        self.metal = 0.0
        self.tempname = 'abc.fits'


def match_stellartype(wave, flux, err):
    return MatchResult()
...
class Spectrum(object):
    ...
    def __init__(self):
        ...
        datadir = os.getenv('PYHAMMER_DATA_PATH')
        if datadir is None:
            homedir = os.getenv('HOME')
            thisdir = os.path.join(homedir, '.spectool', 'PyHammer_data')
        else:
            thisdir = datadir
    ...
    def readSpec(self, wave, flux, err):
    ...
    def getFullTempName(self):
        ...
        #Spectral type SB2
            ...
            return self.thisDir + path_SB2 + tempName
            # return np.nan
        elif bestGuess['specType'] == -1:
            return np.nan
...
def find_stellar_type(wave, flux):
    return True


def get_template_spec(stellartype):
    return wave, flux


def get_temperature(stellartype):
    return 5500


def get_metal(stellartype):
    return 0.2